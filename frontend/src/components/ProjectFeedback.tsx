...
export const ProjectFeedback: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadReviews = async () => {
      try {
        ...
        const payload = await fetchApiJson<FeedbackApiResponse>('/feedback');
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const responsePayload = await fetchApiJson<ReviewEntry>('/feedback', {
        method: 'POST',
        headers: {
  ...
  return (
    <section className="mt-16 grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
      ...
      >
        ...
        <div className="mt-8 space-y-4 max-h-[600px] overflow-auto pr-2">
          {isLoading ? (
          ...
          {reviews.map((review) => (
            <div key={review.id} className="overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/5 to-transparent p-6 transition-all hover:border-white/20 hover:bg-white/8">
              <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex items-center gap-1 mt-2 sm:mt-0">{renderStars(review.rating)}</div>
              </div>

              <div className="space-y-3 border-t border-white/5 pt-4">
                <div>
                  <div className="mb-1.5 text-xs font-semibold uppercase tracking-wider text-cyan-400/70">Description</div>
                  <p className="text-sm leading-relaxed text-gray-400">{review.description}</p>
                </div>
                <div>
                  <div className="mb-1.5 text-xs font-semibold uppercase tracking-wider text-violet-400/70">Expectation</div>
                  <p className="text-sm leading-relaxed text-gray-400">{review.expectations}</p>
                </div>
              </div>
            </div>
          ))}
