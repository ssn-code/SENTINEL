...
export const Compare: React.FC = () => {
  ...
  const [compareParams, setCompareParams] = useState({ stateA: '', stateB: '' });
  ...
  useEffect(() => {
    const update = () => {
      ...
      setCompareParams({
        stateA: params.get("stateA") ?? params.get("a") ?? "",
        stateB: params.get("stateB") ?? params.get("b") ?? "",
      });
    };
  ...
  const findMatchingState = (value: string) => {
    const trimmedValue = value.trim();
    if (!trimmedValue) return undefined;

    return states.find((state) => {
      if (state.id === trimmedValue) return true;
      if (state.state === trimmedValue) return true;
      return normalizeState(state.state) === normalizeState(trimmedValue);
    });
  };
  ...
  useEffect(() => {
    ...
    const matchedA = findMatchingState(compareParams.stateA);
    const matchedB = findMatchingState(compareParams.stateB);

    if (matchedA) {
      setRegionAId(matchedA.id);
    } else if (!regionAId) {
      setRegionAId(states[0]?.id ?? "");
    }

    if (matchedB) {
      setRegionBId(matchedB.id);
    } else if (!regionBId) {
      setRegionBId(states[1]?.id ?? states[0]?.id ?? "");
    }
  }, [compareParams.stateA, compareParams.stateB, regionAId, regionBId, states]);
  ...
  if (loading) {
    return (
      <div className="pt-32 px-6 text-center text-white">
        ...
        <p className="mt-4">Loading state data...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="pt-32 px-6 text-center">
        ...
        <button onClick={() => window.location.reload()} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded">
          Retry
        </button>
      </div>
    );
