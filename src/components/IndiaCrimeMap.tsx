...
export const IndiaCrimeMap: React.FC<IndiaCrimeMapProps> = ({
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (!crimeResponse.ok) throw new Error(`Crime API returned ${crimeResponse.status}`);
        if (!geoJsonResponse.ok) throw new Error(`GeoJSON returned ${geoJsonResponse.status}`);

        const [crimeData, geoJsonData] = await Promise.all([
          crimeResponse.json() as Promise<CrimeMapRow[]>,
          geoJsonResponse.json() as Promise<FeatureCollection<Geometry, IndiaFeatureProps>>,
        ]);
  ...
  return (
    <div className={`relative h-full min-h-[560px] overflow-hidden rounded-[2rem] border border-border bg-[#071019] ${className}`.trim()}>
      <MapContainer
